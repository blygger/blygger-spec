...
| `.responses` | The opt-in public list of verified responses to an item (v0.3). A `<section>` of `<li>` lines — who, at which origin, what relation, when — rendered from local mention rows at request time. Never in `content_html`, the item document, the feed, or a pinned page: it is a view of *other people's* items, and putting it in a versioned document would let a stranger's publish change the author's bytes. |
| `.stub-cite` | A citation of another item, in either of the two senses the client has: a stub's citation of what it answers, and (v0.3 Phase B) a fork's lineage line naming the pinned version it descends from. The leading `.label` distinguishes them ("In response to" / "Forked from"); one class because they are one visual object with one job. Rendered above the body on a permalink and a pinned-version page — lineage also on the withdrawal endcap, which keeps `forked_from`; `.stub-cite.compact` is the one-line form on a feed card and in the RSS description. Presentation, like `.provenance`: the wire carries `stub_of` / `forked_from`, never this markup. The URL is printed as its own anchor text deliberately, so a citation still reads after the link dies. |
