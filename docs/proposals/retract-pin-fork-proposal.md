...
looks, because ygg's data model was already built with half of it in mind (locked decision #2
explicitly cites IPFS-friendliness as the reason for per-version hashes):

| ygg concept | IPFS/IPNS analog | Notes |
|---|---|---|
...
| unpublish-as-endcap | an IPNS record that stops being republished | Diverges from IPFS here: IPNS records have a TTL and expire without active republishing (DHT liveness). ygg's id has no such decay — it's centrally hosted, closer to a **DNS name pointing at one authoritative server** than a DHT record. Durability is "the origin stays up," not "the network keeps propagating the pointer." Don't over-import IPNS's expiry semantics. |
