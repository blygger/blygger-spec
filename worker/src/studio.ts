...
.preview blockquote.ygg-transclusion { margin: 1rem 0; padding: 0.6rem 0.8rem; border-left: 3px solid rgba(128,128,128,0.55); background: rgba(128,128,128,0.08); border-radius: 0 4px 4px 0; font-size: 0.92rem; }
.preview blockquote.ygg-transclusion p { margin: 0 0 0.25rem; }
...
studio.get("/", async (c) => {
  ...
  const body = `${studioHeader("ygg studio")}
<div class="composer">
...
document.getElementById("settings-form").addEventListener("submit", async (e) => {
  ...
  return c.html(studioLayout("settings — ygg studio", body));
});
...
async function fragmentEditPage(db: D1Database, item: ItemRow): Promise<string> {
  ...
  const body = `${studioHeader(`ygg studio — editing ${escapeHtml(item.id.slice(0, 8))}…`)}
<nav style="margin:-0.5rem 0 1rem;font-size:0.9rem;"><a href="/studio">← studio</a> <a href="/ygg/f/${item.id}/" target="_blank">permalink ↗</a></nav>
<div class="split">
<div class="pane">
<h2>markdown</h2>
...
document.getElementById("attach-btn").addEventListener("click", () => {
  ...
  return studioLayout(`editing — ygg studio`, body, true);
}
  ...
  const withdrawBtn =
    item.status === "public"
      ...
      : item.status === "withdrawn"
        ...
        : "";
  const publishLabel = item.status === "withdrawn" || item.version === 0 ? "publish" : `publish v${item.version + 1}`;
  const body = `${studioHeader("ygg studio — editing thread")}
<nav style="margin:-0.5rem 0 1rem;font-size:0.9rem;"><a href="/studio">← studio</a> <a href="/ygg/t/${item.id}/" target="_blank">permalink ↗</a></nav>
...
document.getElementById("attach-btn").addEventListener("click", () => {
  ...
  return studioLayout("editing thread — ygg studio", body, true);
}
