...
- [ ] **`protocol-institute.com/blyg/` node** — the second half of the two-node test network (session 8, Venkat); not yet deployed. DNS for that domain flagged in session 8 as the likely long pole (unlike venkateshrao.com, which was already a Cloudflare zone). Same `docs/deploy-stub-sites-plan.md` mechanics apply once DNS is onboarded.
