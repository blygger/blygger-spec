...
| [`docs/spec-publishing-plan.md`](docs/spec-publishing-plan.md) | PLAN, not executed (session 8, Fable) — Sonnet/Opus-safe: keep `blygger.org/spec/` current via sync script + dated snapshots + git tags; records the spec-as-blygg rejection (decision #15) |
...
1. **Protocol = static file contract** under the blygg origin (mounted anywhere per decision #14; reference default `/blyg/`): `items/{id}.json` is the state plane (ground truth, full archive); `feed.xml` is the notification plane (lossy RSS, just a signal).
...
- [x] **Re-sync `blygger-org/content/spec/0.1/index.md`** — done by the blygger-org agent, verified session 8 (publish copy carries the mount-independence language; site deployed). Hand-copying is retired going forward by the sync script in `docs/spec-publishing-plan.md`.
- [ ] **Execute `docs/spec-publishing-plan.md`** (Sonnet/Opus-safe; blygger-org agent has wrapped, repo is clean — unblocked) — sync script, dated snapshots + `spec/0.1/{date}` git tags, `/spec/` index, deploy.sh secret fix (hardcoded CF token → `Code/.env.keys`; still present as of 2026-08-04, verified never committed/pushed so no rotation forced), then cut snapshot №1 carrying the session-8 mount revision.
- [ ] Deploy targets for the two-node test network (session 8, Venkat): `venkateshrao.com/blyg/` + `protocol-institute.com/blyg/` — supersedes the org/com stub-site pair as the first cross-client deployment; `docs/deploy-stub-sites-plan.md`'s Workers-Routes-over-Pages mechanics apply per domain. blygger.org remains spec/docs home (separate agent building it).
